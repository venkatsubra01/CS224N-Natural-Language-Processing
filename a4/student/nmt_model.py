...
class NMT(nn.Module):
    ...
    def __init__(self, embed_size, hidden_size, vocab, dropout_rate=0.2):
        ...
        # the output of W_u (the below line) times the R^{hx3h} concatenated vector with the decoder hidden state so far (R^{hx1})
        # and the weighted attention vector(R^{3hx1}) (weighted mean of the encoder hidden states (forward and backward) weighted by attention scores)
        # is passed into a tanh and then a softmax to get the PDF over the next target translated word (hx1)
        self.combined_output_projection = nn.Linear(in_features=hidden_size*3, out_features=hidden_size,bias=False)
        # Converts the vector representation from being in hidden state dimension space to vocabulary space
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[
        ...
        ### END YOUR CODE
        # X is shape max_sentence_length (22), batch_size (5), embedding_size (3)
        X = self.model_embeddings.source(source_padded)

        # Convert X to shape batch_size (5), embedding_size (3), max_sentence_length (22)
        X = torch.permute(X, (1,2,0))
        #print(f"source_padded.shape (src_len, b): {source_padded.shape}")

        # Pass X into the CNN to learn features on the scale of 2 words at a time
        X = self.post_embed_cnn(X)

        # Convert X back to original shape
        X = torch.permute(X, (2,0,1))
        packed = pack_padded_sequence(X, source_lengths)

        # enc_hiddens contains the hidden states for all time steps (all words --> size of src_len)
        # last_hidden contains just the final hidden state (just last timestep) for both directions
        # last_hidden/last_cell are the vectors at the last layer of the encoder RNN
        # whereas enc_hiddens will be used for calculating attention scores and the weighted attention vector
        enc_hiddens, (last_hidden, last_cell)  = self.encoder(packed)

        # The second return value of pad_packed_sequence is the original source_lengths that we packed with
        enc_hiddens, _ = pad_packed_sequence(enc_hiddens) # Didn't explicitly check if "each of the
        ###           individual input examples has the same shape."
        ...
        # the initial hidden input to the decoder LSTM
        ...
        # the initial cell input to the decoder LSTM
