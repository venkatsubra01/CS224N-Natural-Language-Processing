...
class CharCorruptionDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        masked_content_length = max(1, min(np.random.binomial(n=truncated_length, p=0.25), truncated_length))
        masked_content_start = int(np.random.uniform(0, truncated_length-masked_content_length))
        
        masked_string_before_pad = (document[:masked_content_start] + self.MASK_CHAR 
                    + document[masked_content_start:masked_content_start+masked_content_length]
                    + self.MASK_CHAR + document[masked_content_start+masked_content_length:])
                
        pads = self.PAD_CHAR * (self.block_size - len(masked_string_before_pad))
        masked_string = masked_string_before_pad + pads
