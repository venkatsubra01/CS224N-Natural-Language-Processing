...
class ParserModel(nn.Module):
    ...
    def __init__(self, embeddings, n_features=36,
        ...
        # Embed to hidden weight matrix is (embed_size*n_features, hidden_size)
        self.embed_to_hidden_weight = nn.Parameter(torch.empty((self.embed_size*n_features, self.hidden_size)))
        # For each node in the hidden layer, we keep a bias
    ...
    def embedding_lookup(self, w):
        ...
        # Printing out the rows of x without list comprehension
        # for indices in w:
        #     list = []
        #     for index in indices:
        #         embedding = torch.index_select(self.embeddings, 0, index)
        #         list.append(embedding)
        #     stack = torch.cat(list, dim=1)
        #     print(f"stack: {stack}")


        # Append all embeddings from features on top of each other (dim=1), then stack all feature embeddings for each batch (dim=0)
        #x = torch.cat([torch.cat([torch.index_select(self.embeddings, 0, index) for index in indices], dim=1) for indices in w])
        # Index into self.embeddings by passing w directly in, and we can reshape from there
        x = self.embeddings[[w]].view(w.shape[0],-1) # this is significantly faster than using list comprehension
        return x
    ...
    def forward(self, w):
        ...
        # More common to do this in __init__ but this works
        m = nn.ReLU()
        # Apply dropout after ReLU as described in above instructions
        h = self.dropout(m(torch.matmul(x, self.embed_to_hidden_weight)+self.embed_to_hidden_bias))
