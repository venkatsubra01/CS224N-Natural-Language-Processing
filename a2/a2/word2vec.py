...
def naiveSoftmaxLossAndGradient(
    ...
    ### to integer overflow. 
    y_hat = softmax(outsideVectors @ centerWordVec).T
    loss = (-1)*np.log(y_hat[outsideWordIdx]) # softmax goes over all rows
    
    # make the one-hot vector y
    y = np.zeros(len(outsideVectors))
    y[outsideWordIdx] = 1
    gradCenterVec = (y_hat - y) @ outsideVectors
    gradOutsideVecs = y_hat.reshape(-1,1) @ centerWordVec.reshape(1,-1)
...
def negSamplingLossAndGradient(
    ...
    # This needs to be written to be more efficient
    ...
    U_original = outsideVectors[indices]
    U = outsideVectors[indices]
    U[1:] = (-1)*U[1:] # Flip the direction of the neg sampled words for convenience later (don't have to do -u explicitly in the sigmoids)

    # This term will be reused
    S = sigmoid(U @ centerWordVec)

    loss = (-1)*np.log(S[0]) - np.sum(np.log(S[1:]))
    # (S[0]-1)*outsideVectors[outsideWordIdx] = (sigmoid(u_o^Tv_c)-1)u_o
    gradCenterVec = (S[0]-1)*outsideVectors[outsideWordIdx] + np.sum(np.diag(1-S[1:])@(U_original[1:]),axis=0) #  np.sum(np.diag(1-S[1:])@(U_original[1:]),axis=0) = \sum_{s=1}^K (1-sigmoid(-u_{w_s}^Tv_c))u_{w_s}
    unique, frequency = np.unique(negSampleWordIndices, return_counts = True)
    
    # Make a vector of counts (# of words in vocab, 1) that counts the # of times each index is in the neg sampled
    counts = np.zeros(len(outsideVectors))
    counts[unique] = frequency
    gradOutsideVecs = np.zeros(outsideVectors.shape) # (# of words in vocab, word vector length)
    # For each negative sampled word index, do # of occurrences of that index * (1-sigmoid(-u_{w_s}^Tv_c))*v_c
    gradOutsideVecs[negSampleWordIndices] = np.diag(counts[negSampleWordIndices]) @ ((1-S[1:]).reshape(-1,1) @ centerWordVec.reshape(1,-1))
    # (sigmoid(u_o^Tv_c)-1)v_c
    gradOutsideVecs[outsideWordIdx] = (S[0]-1)*centerWordVec
    ### END YOUR CODE
...
def skipgram(currentCenterWord, windowSize, outsideWords, word2Ind,
    ...
    for word in outsideWords:
        this_loss, gradCenterVec, this_gradOutsideVectors = word2vecLossAndGradient(centerWordVectors[word2Ind[currentCenterWord]], word2Ind[word], outsideVectors, dataset)
        loss += this_loss
        gradOutsideVectors += this_gradOutsideVectors
